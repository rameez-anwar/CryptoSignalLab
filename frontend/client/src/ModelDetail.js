...
function ModelDetail({ tableName, onBack }) {
  ...
  const fetchLedgerData = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        const allData = response.data.data.ledger || [];
        setAllLedgerData(allData);
  ...
  const handleItemsPerPageChange = (newItemsPerPage) => {
    ...
    setTotalPages(Math.ceil(allLedgerData.length / newItemsPerPage));
  };
  ...
  // Recalculate total pages when data or items per page changes
  useEffect(() => {
    if (allLedgerData.length > 0) {
      setTotalPages(Math.ceil(allLedgerData.length / itemsPerPage));
    }
  }, [allLedgerData, itemsPerPage]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-gradient-to-br from-white via-gray-50 to-green-50 rounded-2xl shadow-2xl p-8 mt-8 border border-gray-100">
          <div className="flex items-center space-x-4 mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-1">Trade Ledger</h2>
              <p className="text-gray-600">Detailed transaction history and trade performance</p>
            </div>
          </div>
          ...
          {ledgerLoading ? (
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600 mx-auto"></div>
                <p className="mt-4 text-gray-600">Loading trade ledger...</p>
              </div>
            </div>
          ) : allLedgerData && allLedgerData.length > 0 ? (
            <div className="space-y-6">
                ...
                <div className="px-6 py-4 bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-200">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">All trades and their performance</p>
                    </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {allLedgerData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((trade, index) => (
                        <tr 
                          key={index}
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            ${parseFloat(trade.buy_price || 0).toFixed(2)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            ${parseFloat(trade.sell_price || 0).toFixed(2)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`text-sm font-bold ${
                              parseFloat(trade.pnl_percent || 0) >= 0 
                                ? 'text-green-600' 
                                : 'text-red-600'
                            }`}>
                              {parseFloat(trade.pnl_percent || 0) >= 0 ? '+' : ''}
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            ${parseFloat(trade.balance || 0).toFixed(2)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-4">
                        <span className="text-sm text-gray-700">
                          Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, totalRecords)} of {totalRecords} trades
                        </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                  <div className="text-sm text-gray-500 mb-1">Winning Trades</div>
                  <div className="text-xl font-bold text-green-600">
                    {allLedgerData.filter(trade => parseFloat(trade.pnl_percent || 0) > 0).length}
                  </div>
                </div>
                <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                  <div className="text-sm text-gray-500 mb-1">Losing Trades</div>
                ...
                <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                  ...
                  <div className="text-xl font-bold text-purple-600">
                    {totalRecords > 0 
                      ? ((allLedgerData.filter(trade => parseFloat(trade.pnl_percent || 0) > 0).length / totalRecords) * 100).toFixed(1)
                      : 0}%
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <h4 className="text-lg font-semibold text-gray-700 mb-2">No Trade Data</h4>
                <p className="text-gray-500 text-sm max-w-md">
                  Trade ledger data will appear here once transactions are available for this model
                </p>
              </div>
            </div>
          )}
